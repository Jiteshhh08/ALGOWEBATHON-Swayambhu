export function computeMetrics({ requests, hospitals, ambulances, routeStats }) {
  const totalReq = requests.length
  const avgWait = totalReq ? (requests.reduce((s, r) => s + (Date.now() - r.createdAt) / 60000, 0) / totalReq) : 0

  const crit = requests.filter(r => r.urgency === 'Critical')
  const avgCritWait = crit.length ? (crit.reduce((s, r) => s + (Date.now() - r.createdAt) / 60000, 0) / crit.length) : 0
